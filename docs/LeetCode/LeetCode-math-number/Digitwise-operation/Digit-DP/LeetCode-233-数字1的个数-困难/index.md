...
考察的是数的构成原理，考察抽象的能力，非常好的题目。

一、基于**数位**而不是基于**数**的统计方式

需要注意的是，题目要求统计的是1的个数，而不是数的个数。相比于暴力地逐个数逐个数地枚举，枚举每一数位上 1 的个数，然后使用乘法原理，能够快速的统计出在该数位为1且位于区间内的数字的个数，这是非常高效的。

二、乘法原理

基于数位的计数问题是非常典型的运用乘法原理的，举个例子，要统计百位上为1的数的个数，即要统计如下形式：

```
**1**
```

固定一位，由它的高位、低位进行组合。下面的例子来进行理解:
...
### [官方解题](https://leetcode.cn/problems/number-of-digit-one/solution/shu-zi-1-de-ge-shu-by-leetcode-solution-zopq/) # 方法一：枚举每一数位上 1 的个数
...
> NOTE: 
>
> 一、上面这种讨论仅仅局限于统计"**1**" 的个数，那如果推广到统计任意digit，应该如何来设计算法呢？
>
> 
...
public:
  int countDigitOne(int n)
  {
    ...
    {
      long long divider = mulk * 10;                                             // 除数，10^{k+1}
...
一、当 $c$ 前面的部分 $< ab$，即范围为 $[0, ab)$，此时必然满足「大小要求」，因此后面的部分可以任意取，即范围为 $[0, 99]$。根据「乘法原理」，可得知此时数量为 $ab * 100$；
...
二、当 $c$ 前面的部分 $=ab$，这时候「大小关系」主要取决于 $c$：
...
三、当 $c$ 前面的部分 $>ab$，必然不满足「大小要求」，数量为 0。
