...
Step 1: 基于**数位**而不是基于**数**的统计方式

需要注意的是，题目要求统计的是1的个数，而不是数的个数。

相比于暴力地逐个数逐个数地枚举，枚举每一数位上 1 的个数，能够快速的统计出在该数位为1且位于区间内的数字的个数，这是非常高效的

这道题是要求解从1到n的各个数每一位上1的个数的总数，这种思路本质上是：基于**数位**而不是基于**数**的统计方式，这是一种高效的穷举，这一类问题都是基于数位的：

基于数位的方式，便于利用乘法原理来进行计数。
