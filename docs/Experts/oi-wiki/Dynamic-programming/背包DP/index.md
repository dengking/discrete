...
>
> "$f_{i,j-w_i}$ 已经由 $f_{i,j-2\times w_i}$ 更新过，那么 $f_{i,j-w_i}$ 就是充分考虑了第 $i$ 件物品所选次数后得到的最优结果" 结合具体的例子来理解这段话: $f_{3, 5}$ 从 $f_{3, 4}$ 转移而来，这就相当于第i个物品被重复选择了。
>
> 这应该就是这段话中的 **局部最优子结构** 的含义。
>
> 三、虽然是可以无限的取，但是还是受背包容量的限制，所以实际只能够取有限次数
