...
class Solution:
    def reverseBetween(self, head: Optional[ListNode], left: int, right: int) -> Optional[ListNode]:
        """
        找到第left node的prev、第right node的next
        reverse [left, right] 范围内的node
        将reverse部分重新接入到原list中
        :param head:
        :param left:
        :param right:
        :return:
        ...
        cur = prev_left.next  # 经过反转，它就成为了反转部分的tail node
        # 让cur的next指向第right+1个node
        prev_left.next, cur.next = self.reverse_linked_list(cur, right - left + 1)
        return dummy.next

    @staticmethod
    def reverse_linked_list(head: Optional[ListNode], reverse_node_cnt) -> Tuple[
        Optional[ListNode], Optional[ListNode]]:
        prev, cur, next_node = None, head, None
        cnt = 0
        while cur and cnt < reverse_node_cnt:
            next_node = cur.next
            cur.next = prev
            prev = cur
            cur = next_node
...
![](/Users/kaideng/Documents/GitHub/discrete/docs/Data-structure/Linked-list/Algorithm/LeetCode/Reverse-linked-list/LeetCode-92-反转链表-II-中等/LeetCode-92-Reverse-linked-list-II-中等.png)
