...
一、一个node，如果它的 "入度"  为0，则表示它没有依赖其他节点，它是**低阶课程**，应该优先学习

BFS优先寻找**低阶课程**，将它放到队首；

BFS中只需要**入度**即可: 它根据**入度**来寻找**低阶课程**；某个节点的入度如果非0，则表示它存在依赖；

BFS使用queue；

BFS其实是**正向排序**，它借助queue；

BFS不需要处理circle deadloop

二、一个node，如果它的 "出度"  为0，则表示没有其它节点依赖它，它是**高阶课程**，应该最后学习

DFS优先寻找**高阶课程**，将它放到栈底；

DFS中只需要**出度**即可: 它根据**出度**来寻找**高阶课程**；

DFS使用stack；

DFS需要处理circle deadloop: 如果存在circle，那么在DFS circle的时候，肯定会回到原点，因此可以据此来判定是否有circle，一旦有circle，则直接break即可。

DFS其实是**逆向排序**，再借助stack进行逆向，从而可以得到正确的顺序
