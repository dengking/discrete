...
思考: 能否改变上述nested-for的方式，将"for 体积"和"for 物品"调换？这对结果有啥影响？经过反复思考: 上述写法是由背包问题的**状态转移方程**而决定的。
...
| j=2  |      |                                             |
|      | i=0  |                                             |
|      | i=1  |       dp[2]=max(dp[2],dp[2-2]+40)=40        |
|      | i=2  |                                             |
|      | i=3  |       dp[2]=max(dp[2],dp[2-2]+50)=50        |
| j=3  |      |                                             |
|      | i=0  |       dp[3]=max(dp[3],dp[3-3]+40)=40        |
|      | i=1  |       dp[3]=max(dp[3],dp[3-2]+40)=40        |
|      | i=2  |                                             |
|      | i=3  |       dp[3]=max(dp[3],dp[3-2]+50)=50        |
| j=4  |      |                                             |
|      | i=0  | dp[4]=max(dp[4],dp[4-2]+40)=50+40(逻辑错误) |
|      | i=1  |                                             |
|      | i=2  |                                             |
|      | i=3  |                                             |
| j=5  |      |                                             |
...
对于0-1背包，它的内循环是从大到小，第i个物品只能够取一次

对于完全背包，它的内循环是从小到大，第i个物品是可以重复取的
