...
格中的**不动点**，本质是**格上单调函数迭代收敛后得到的稳定解**，分为**最小不动点（LFP）**和**最大不动点（GFP）**两类，是自动机ε闭包、数据流分析的统一数学基础。

---

## 一、数学定义

设 $(L, \sqsubseteq)$ 是**完备格**，$f: L \to L$ 是**单调函数**（$a \sqsubseteq b \implies f(a) \sqsubseteq f(b)$）：

- **不动点**：满足 $\boldsymbol{f(x) = x}$ 的元素 $x \in L$，即函数作用后值不发生变化。
- **Knaster–Tarski 定理**：完备格上的任意单调函数，其所有不动点仍构成一个完备格，必然存在**最小不动点 $\text{lfp}(f)$** 和**最大不动点 $\text{gfp}(f)$**。
...
| 类型            | 定义                                                          | 迭代方式                                       | 核心语义                                          |
| ------------- | ----------------------------------------------------------- | ------------------------------------------ | --------------------------------------------- |
| **最小不动点 LFP** | 所有不动点中最小的元素   （对任意不动点 $x$，都有 $\text{lfp}(f) \sqsubseteq x$） | 自底向上迭代：从格的底元 $\bot$ 出发，反复应用 $f$，序列单调递增直到稳定 | **从无到有，收集所有「可到达、可能成立」的信息**（存在性 / may 分析）      |
| **最大不动点 GFP** | 所有不动点中最大的元素   （对任意不动点 $x$，都有 $x \sqsubseteq \text{gfp}(f)$） | 自顶向下迭代：从格的顶元 $\top$ 出发，反复应用 $f$，序列单调递减直到稳定 | **从全到精，保留所有「必然成立、所有路径都满足」的信息**（全称性 / must 分析） |
...
- 格：状态集合的幂集格 $\mathcal{P}(Q)$，偏序为集合包含 $\subseteq$

- 函数：$f(S) = S \cup { q \mid \exists p \in S,\ p \stackrel{\varepsilon}{\to} q }$

- 结果：从空集迭代收敛得到的**LFP**，就是从起点仅靠ε转移能到达的全部状态。> 
  
  > 子集构造、NFA可达状态集的计算，本质都是求幂集格上的最小不动点。
...
| 数据流分析 | 不动点类型 | 分析性质       | 说明                     |
| ----- | ----- | ---------- | ---------------------- |
| 到达定值  | LFP   | may / 存在性  | 只要有一条路径能将定值带到该点，就算到达   |
| 活跃变量  | LFP   | may / 存在性  | 只要有一条后续路径使用该变量，就算活跃    |
| 可用表达式 | GFP   | must / 全称性 | 所有到达该点的路径都计算过该表达式，才算可用 |
| 常量传播  | GFP   | must / 全称性 | 所有路径上变量都取同一个值，才算常量     |

---

## 四、和 LCA 的区分

你之前问的**最近公共祖先（LCA）**，是树偏序格中**两个元素的下确界（$\(\inf)$）**，属于**二元运算**的结果；
而**不动点**是**一元单调函数**的稳定解，二者不是同一层级的概念。

- 格的二元运算：交（$\sqcap$，对应LCA）、并（$\sqcup$，对应最近公共后代）
- 格的函数不动点：LFP、GFP，是迭代算法的收敛解
